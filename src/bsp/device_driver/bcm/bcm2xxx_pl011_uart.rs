...
impl console::interface::Read for PL011Uart {
    fn read_char(&self) -> char {
        self.inner.lock(|inner| inner.read_char(BlockingMode::Blocking).expect("when using BlockingMode::Blocking, a character should always be read"))
    }

    fn clear_rx(&self) {
        while self.inner.lock(|inner| inner.read_char(BlockingMode::NonBlocking)).is_some() {}
    }
}
